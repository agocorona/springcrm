...
class CustomerRestController {
	...
	// if there is no id, the register is identified by the current customer name
    ...
    void modifyCustomer(Principal principal, @RequestBody Customer input) {
		...
		Customer customerToUpdate=customerRepository
					...
					.orElseGet(() -> customerRepository.findByName(input.getName())
					.orElseThrow(() -> new CustomerNotFoundException(input.getName())));
	...
	// if the id is not present, the register is identified by the user name.
